...
| `hook_delivery/` | Cómo se resuelven y renderizan los hooks/settings que hoy se le pasan a Claude (`--settings`) y Codex (`codexHooksConfig`) | Consultar solo si un runner nuevo necesita entender cómo se arma ese archivo temporal — no es el foco central |
