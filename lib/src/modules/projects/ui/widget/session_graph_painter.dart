...
class SessionGraphPainter extends CustomPainter {
  ...
  List<_Placed> _layout(Size size) {
    final depth = <String, int>{SessionGraphNode.userId: 0};
    for (final edge in graph.edges) {
      ...
      final next = edge.kind == SessionGraphEdgeKind.consult ? base : base + 1;
      depth[edge.toId] = math.max(depth[edge.toId] ?? 0, next);
    }

    final byDepth = <int, List<SessionGraphNode>>{};
