...
/// The station palette, taken from the mockup. It is deliberately *not* the
...
/// four-member station, so a typical station looks exactly like the design.
const kStationMemberPalette = <Color>[
...
/// The colour that identifies a member inside one station, assigned by its
/// position in the member list. Past the palette it wraps, which is fine: a
/// station with more than eight members is well beyond what the layout — a
/// facepile and a step list — is meant to show at a glance.
Color memberColorFor(int memberIndex) {
  if (memberIndex < 0) return kStationMemberPalette.first;
  return kStationMemberPalette[memberIndex % kStationMemberPalette.length];
}

/// The palette slot for whoever wrote a message. Current members keep their
/// position-based colour so a station looks like the design; an author that is
...
int authorPaletteIndex(String profileId, List<String> memberIds) {
  ...
  return hash % kStationMemberPalette.length;
}
